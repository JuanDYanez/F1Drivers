import { GET_DRIVERS, GET_DRIVER_BY_NAME, GET_TEAMS, FILTER_BY_TEAMS, FILTER_BY_DB, ORDER_BY_NAME, ORDER_BY_DOB } from "./actions-types"
  ...
  switch (action.type) {
    ...
    case FILTER_BY_TEAMS:
      ...
      let teamsFilter = state.drivers.filter((driver) => {
        ...
        if (driver.createdInDB) {
          return driver.Teams && driver.Teams.some((team) => team.name === action.payload)
        }
        return false;
      })
      return {
    ...
    case ORDER_BY_NAME:
      ...
      if (action.payload === "A") {
        ...
        return a.forename.localeCompare(b.forename, 'en', { sensitivity: 'base' });
        })
      }
      if (action.payload === "D") {
        orderedByName = [...state.drivers].sort((a, b) => {
        return b.forename.localeCompare(a.forename, 'en', { sensitivity: 'base' });
        })
      }
      return {
        ...state,
